...
dropout_P = 0.3 # EqT uses 0.1, SWAG 0.05 (best performace out of 0.1, 0.05, 0.01)

class CNNNetDropout(torch.nn.Module):

    def __init__(self, num_channels=1, min_lag = -0.5, max_lag = 0.5):
        super(CNNNetDropout, self).__init__()
        ...
        print("Dropout rate is:", dropout_P)
    ...
    def forward(self, x):
        ...
        # EqT uses SpatialDropout1D (Keras) for convolutional layers - Droupout1D (pytorch) seems similar but is
        # not available in my version. Use regular dropout layer (like SWAG paper implementation) or use not dropout 
        # for convolutional layers?

        # First convolutional layer
        # x = self.dropout(x) # First layer is dropout in SWAG VGG_dropout - weird? - Not in PreResNet or WideResNet
        x = self.conv1(x)
        x = self.relu(x)
        x = self.bn1(x)
        x = self.maxpool(x)
        ...
        x = self.dropout(x) # Dropout before each weight layer, in SWAG VGG_dropout is after max pool
        x = self.conv2(x)
        x = self.relu(x)
        x = self.bn2(x)
        x = self.maxpool(x)
        ...
        x = self.dropout(x) # Dropout before each weight layer
        x = self.fcn1(x)
        x = self.relu(x)
        x = self.bn4(x)
        ...
        # Last layer
        # SWAG VGG_dropout doesn't have dropout before the final linear layer- does appear to be one in SWAG PreResNet & WideResNet
        x = self.dropout(x) # Dropout before each weight layer
        x = self.fcn3(x)
