...
dropout_P = 0.05 # EqT uses 0.1, SWAG 0.05 (best performace out of 0.1, 0.05, 0.01)

class CNNNetDropoutFC(torch.nn.Module):

    def __init__(self, num_channels=1, min_lag = -0.5, max_lag = 0.5):
        super(CNNNetDropoutFC, self).__init__()
...
class PPickerDropoutFC:
    base = CNNNetDropoutFC
    args = list()
    kwargs = {"num_channels":1, "min_lag":-0.75, "max_lag":0.75}

class SPickerDropoutFC:
    base = CNNNetDropoutFC
    args = list()
