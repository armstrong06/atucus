...
def train_epoch(
    ...
    residuals = np.zeros(len(loader.dataset))
    ...
    for i, (input, target) in enumerate(loader):
        ...
        if not regression:
            ...
            pred = output.data
            resids = target.sub(pred.data.view_as(target))
            residual_std += resids.sum().item()
            residual_mean += resids.std().item()
            residuals[index:index+len(input)] = resids.cpu().data.numpy()[:, 0]
...
def eval(loader, model, criterion, cuda=True, regression=False, verbose=False):
    ...
    residuals = np.zeros(num_objects_total)
    ...
    # print((target-pred).cpu().data.numpy(), resids.cpu().data.numpy())
    return {
