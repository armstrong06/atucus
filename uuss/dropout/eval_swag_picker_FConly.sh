...
for seed in ${seeds[@]}; do
	...
	for split in ${splits[@]};do
		...
		--validation_dataset="uuss_${split}_4s_1dup.h5" --model="PPickerDropoutFC"\
                --batch_size=${batch_size} --method="Dropout" --cov_mat --scale=0.5 --seed=${seed} \
                --save_path="${dir}/eval_checkpoint${checkpoint}_dropout${dropout_rate}_FConly_N${N}/dropout_${split}_uncertainty_${checkpoint}" \
	       	--max_num_models=$K --N=$N &
        wait -n 
        wait
	done
done
