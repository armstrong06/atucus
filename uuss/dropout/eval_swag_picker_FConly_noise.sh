...
for seed in ${seeds[@]}; do
	...
	for split in ${splits[@]};do
		...
		--validation_dataset="YSnoiseZ_4s.h5" --model="PPickerDropoutFC"\
                --batch_size=${batch_size} --method="Dropout" --cov_mat --scale=0.5 --seed=${seed} \
                --save_path="${dir}/eval_checkpoint${checkpoint}_dropout${dropout_rate}_FConly_N${N}_noise/dropout_${split}_uncertainty_${checkpoint}" \
